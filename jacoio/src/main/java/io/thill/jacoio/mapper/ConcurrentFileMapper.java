...
public class ConcurrentFileMapper {
    ...
    if(roll.enabled) {
      ...
      if(multiProcess) {
        ...
        mappedFileProvider = new MultiProcessMappedFileProvider(roll.coordinationFile, capacity, fillWithZeros, roll.fileProvider,
                roll.yieldOnAllocateContention, roll.preallocate, roll.preallocateCheckMillis, roll.fileCreatedListener);
      } else {
        mappedFileProvider = new SingleProcessMappedFileProvider(capacity, fillWithZeros, roll.fileProvider, roll.yieldOnAllocateContention, roll.preallocate,
                roll.preallocateCheckMillis, roll.fileCreatedListener);
      }
      final RollingCoordinator rollingCoordinator = new RollingCoordinator(mappedFileProvider, roll.yieldOnAllocateContention, roll.asyncClose,
              roll.fileMappedListener, roll.fileCompleteListener, roll.fileClosedListener);
  ...
  public class RollParameters {
    ...
    private FileCreatedListener fileCreatedListener;
    private FileMappedListener fileMappedListener;
    private FileCompleteListener fileCompleteListener;
    ...
    /**
     * Optional function to run on every file after it has been created, but before it is cycled in for use. Called from the thread that
     * created/mapped/allocated the file as soon as it is created. In multi-process mode, this callback will only be generated by the process that physically
     * created the file.
     *
     * @param fileCreatedListener
     * @return
     */
    public RollParameters fileCreatedListener(FileCreatedListener fileCreatedListener) {
      this.fileCreatedListener = fileCreatedListener;
      return this;
    }

    /**
     * Optional function to run on every file after it has been mapped, but before any write is performed from this process. Called from the thread that cycles
     * the file in for use, immediately before it is first used by this process.
     *
     * @param fileMappedListener
     * @return
     */
    public RollParameters fileMappedListener(FileMappedListener fileMappedListener) {
      this.fileMappedListener = fileMappedListener;
      return this;
    }

    /**
     * Optional function to run on every file after it has been completed, but before it is closed. Called from the thread that closes the file.
     *
     * @param fileCompleteListener
     * @return
     */
    public RollParameters fileCompleteListener(FileCompleteListener fileCompleteListener) {
      this.fileCompleteListener = fileCompleteListener;
      return this;
    }

    /**
     * Optional function to run on every file after it has been closed. Called from the thread that closes the file.
     *
     * @param fileClosedListener
     * @return
     */
    public RollParameters fileClosedListener(FileClosedListener fileClosedListener) {
